...
func executar() {
	...
	for {
		...
		// CHAMADA MÁGICA: Uma linha substitui todas as outras
		mostrarRelatorio(u)

	} // fim do loop

}
func mostrarRelatorio(u GithubResponse) {
	...
	fama := calcularPontos(u) // A variável 'fama' recebe o número que voltou

	fmt.Printf("🏆  PONTUAÇÃO DE FAMA: %d pontos\n", fama)

	// Seu IF antigo pode usar essa pontuação agora!
	if fama > 1000 {
		fmt.Println("🔥 Nível: LENDÁRIO!")
	} else if fama > 500 {
		fmt.Println("💎 Nível: Diamante")
	} else {
		fmt.Println("🪵 Nível: Madeira")
	}
...
func calcularPontos(u GithubResponse) int {
	// Cada repositório vale 10 pontos, cada seguidor vale 1 ponto
	pontos := (u.PublicRepos * 10) + u.Followers

	return pontos // <--- Aqui ela devolve o resultado para quem chamou
}
