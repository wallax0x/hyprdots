...
def dashboard():
    ...
    # 1. Busca contagem de Produtos
    total_produtos = conn.execute('SELECT COUNT(id) FROM produto').fetchone()[0]
    
    # 2. Busca contagem de Categorias
    total_categorias = conn.execute('SELECT COUNT(id) FROM categoria').fetchone()[0]
    
    # 3. Busca contagem de Usuários (ISSO FALTAVA)
    total_usuarios = conn.execute('SELECT COUNT(id) FROM usuario').fetchone()[0]

    # 4. Busca contagem de Produtos Ativos (Para substituir a Taxa de Rejeição)
    # Obs: Se no seu banco o ativo for salvo como 1 ou 0, mude "True" para 1.
    produtos_ativos = conn.execute('SELECT COUNT(id) FROM produto WHERE ativo = ?', ("True",)).fetchone()[0]
    ...
    return render_template('admin/dashboard.html',
                            ...
                            total_usuarios=total_usuarios, # Agora estamos enviando!
                            produtos_ativos=produtos_ativos, # Dado novo
