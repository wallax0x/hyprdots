...
def dashboard():
    ...
    # Mudamos de p.categoria_id para p.id_categoria
    query_grafico = '''
...
def listarCategoria():
    ...
    # Envia para o template (você precisa ter o arquivo listar_categoria.html)
...
def excluirUsuario(id):
    conn = conexao()
    # Mudei o nome da variável para 'usuario'
    usuario = conn.execute('SELECT * FROM usuario WHERE id = ?', (id,)).fetchone()
    ...
    # Agora sim a variável 'usuario' existe
    return render_template('admin/excluir_usuario.html', usuario = usuario )
