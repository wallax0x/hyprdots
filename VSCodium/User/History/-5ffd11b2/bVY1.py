...
def dashboard():
    ...
    # 1. Busca os totais simples (que você já tinha)
    ...
    # 2. Busca dados para o gráfico (Produtos por Categoria)
    # O LEFT JOIN garante que categorias sem produtos apareçam com contagem 0 (se houver tratamento)
    # Se quiser mostrar apenas categorias que TEM produtos, troque LEFT JOIN por JOIN
    query_grafico = '''
        ...
        GROUP BY c.id
    '''
    resultado_grafico = conn.execute(query_grafico).fetchall()
    
    conn.close()
    
    # 3. Prepara os dados para o Chart.js (transforma as linhas do banco em listas)
    # linha[0] é o nome da categoria, linha[1] é a contagem
    labels_grafico = [linha[0] for linha in resultado_grafico] 
    data_grafico = [linha[1] for linha in resultado_grafico]
    
    # 4. Envia tudo para o template
    return render_template('admin/dashboard.html',
                            total_produtos=total_produtos,
                            total_categorias=total_categorias,
                            labels_grafico=labels_grafico, # Passando os nomes
                            data_grafico=data_grafico)     # Passando os números
