...
def cadastrarProduto():
    ...
    categorias = conn.execute('SELECT * FROM categoria WHERE ativo = "True"').fetchall()
    conn.close()
    return render_template('admin/cadastrar_produto.html', categorias=categorias)


# Rota para LISTAR Produtos (Sua solicitação)
@app.route("/listarprodutos")
def listarProdutos():
    conn = conexao()
    # SQL corrigido para seu schema (produto, categoria, id_categoria)
    # Também buscamos p.imagem para exibir na lista
    sql_query = """
...
def detalhesProduto(id):
    ...
    # Vamos usar um JOIN para pegar o produto E o nome da sua categoria
    sql_query = """
