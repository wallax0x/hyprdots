...
func homeHandler(c *gin.Context) {
	// 1. Busca as vagas (igual antes)
	var vagas []models.Vaga
	database.DB.Preload("Empresa").Order("created_at desc").Find(&vagas)

	// 2. VERIFICAÇÃO "SILENCIOSA" DO LOGIN
	// Tenta ler o cookie. Se conseguir, isLogado vira true.
	_, err := c.Cookie("empresa_id")
	isLogado := (err == nil)

	// 3. Manda tudo pro HTML (Vagas + Status do Login)
	c.HTML(http.StatusOK, "home.html", gin.H{
		"vagas":    vagas,
		"isLogado": isLogado, // <--- O HTML vai usar isso para mudar o botão
	})
}
