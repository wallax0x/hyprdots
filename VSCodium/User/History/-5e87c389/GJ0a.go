...
func main() {
	...
	// --- ROTAS DE PERFIL DO CANDIDATO ---

	// 1. Mostrar o Perfil
	r.GET("/candidato/perfil", func(c *gin.Context) {
		...
		// Busca o candidato no banco para preencher o formulário
		if err := database.DB.First(&candidato, candidatoID).Error; err != nil {
			c.Redirect(http.StatusFound, "/candidato/login")
			return
		}

		c.HTML(http.StatusOK, "perfil_candidato.html", gin.H{
			"candidato":   candidato,
			"isCandidato": true,
		})
	})
	...
	r.POST("/candidato/perfil", func(c *gin.Context) {
		candidatoID, _ := c.Cookie("candidato_id")

		// Busca o candidato atual
		var candidato models.Candidato
		database.DB.First(&candidato, candidatoID)

		// Atualiza campos de texto
		candidato.Nome = c.PostForm("nome")
		candidato.Telefone = c.PostForm("telefone")
		candidato.Skills = c.PostForm("skills")

		// Verifica se mandou NOVO arquivo
		file, errFile := c.FormFile("curriculo")
		if errFile == nil {
			// Se mandou arquivo novo, faz o upload e substitui o link
			nomeArquivo := fmt.Sprintf("%d_%s", time.Now().Unix(), file.Filename)
			caminhoArquivo := "uploads/" + nomeArquivo
			c.SaveUploadedFile(file, caminhoArquivo)

			candidato.ArquivoCurriculo = caminhoArquivo
		}

		// Salva no banco
		database.DB.Save(&candidato)

		// Volta pro dashboard avisando que deu certo (opcional)
		c.Redirect(http.StatusFound, "/candidato/dashboard")
	})

	// 3. Deletar Conta (Perigo!)
	r.POST("/candidato/delete", func(c *gin.Context) {
		candidatoID, _ := c.Cookie("candidato_id")

		// 1. Apagar candidaturas desse usuário (Opcional, mas bom pra limpar)
		database.DB.Where("candidato_id = ?", candidatoID).Delete(&models.Candidatura{})

		// 2. Apagar o Usuário
		database.DB.Delete(&models.Candidato{}, candidatoID)
